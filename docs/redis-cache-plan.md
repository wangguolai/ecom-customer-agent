...
### 决策 7：防护分层——穿透/击穿/雪崩已落地，退款分布式锁仍认知为主
- **穿透**：空值哨兵 `set_empty`（`__EMPTY__`），已落地（模块 3 原样）。
- **击穿**：agent 侧 `asyncio.gather` 并发查同一 key（用户同时问订单+物流）是「击穿微缩场景」。**2026-09-04 补落地**：`cache.get_or_rebuild`（Redis SET NX EX 重建锁 + 双重检查 + 锁超时/降级），同一时刻只有一个线程回源 DB，其余重试读缓存。对抗测试 `tests/test_cache_stampede.py`（20 并发 miss 回源仅 1 次）。
- **雪崩**：**2026-09-04 补落地**：`_jittered_ttl` TTL 随机抖动 ±5s（`randint` 闭区间对称），避免同类/热点 key 同一秒集体过期。
- **分布式锁（退款幂等）**：退款幂等仍在模块 2/5 用 DB `UNIQUE(order_id)` 唯一约束（单机单进程内是正解），分布式锁只在「多实例部署」下才有意义，demo 无多实例 → 退款这条仍认知不落地。⚠️ 规范别自相矛盾：击穿互斥锁本身就是 Redis 分布式锁的一种应用，说「分布式锁没落地」要限定在「退款幂等场景」，别和击穿互斥锁打架。
