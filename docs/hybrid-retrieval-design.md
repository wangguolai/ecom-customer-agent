...
纯向量检索按语义走，对「精确词」（品牌名「皇家牌」、特征词「肠胃敏感」）在**正确类别内**也可能漏掉——因为向量按语义相似度，不按关键词精确匹配。

> ⚠️ 注意：不是「问猫粮漏掉犬粮」那种跨类别漏（那是对的，锁类别优先）。是「问皇家牌，向量可能漏掉皇家牌自己的商品」这种**同类别内**的精确词漏召回。
...
- jieba 配置：`jieba.add_word("皇家牌")` 等，把品牌名、专有名词加进词典（否则「皇家牌」被切成「贝/乐/牌」，匹配不上）
...
1. 「皇家牌有哪些粮」→ 精确召回皇家牌的 **2 款**狗粮（幼犬成长粮 + 成犬均衡粮），之前纯向量可能漏
